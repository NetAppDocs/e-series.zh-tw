...
[role="lead"]
下表描述了最新 SANtricity OS 版本中針對 EF80、EF50、E4000、EF300、EF600、E2800/EF280 和 E5700/EF570 控制器的嵌入式管理的新功能。
...
NOTE: SANtricity OS 12.0 版本僅相容於新型 EF80 和 EF50 全快閃儲存系統。
...
新型儲存系統型號 – EF80 和 EF50 全快閃儲存系統
 a| 
此版本推出了 EF80 和 EF50 全 NVMe 快閃儲存系統。EF80 和 EF50 每個控制器支援 24 個 NVMe SSD 驅動器和一個主機介面卡（HIC）。支援的 NVMe over Fabrics 主機介面包括 NVMe over RoCE、NVMe over IB（適用於 EF80）和 NVMe over FC（適用於 EF50）。支援的 SCSI 介面包括 Fibre Channel。

EF80 和 EF30 也支援 30TB 或 60TB 大容量的 NVMe SSD 磁碟機。EF80 和 EF30 僅相容於 Dynamic Disk Pool 使用，不支援傳統 RAID。
...
SANtricity System Manager 現已提供指令碼編輯器
 a| 
SANtricity System Manager 中現已提供全新的腳本編輯器。腳本編輯器可讓您建立或編輯腳本檔案、將腳本檔案儲存至儲存管理站的本機磁碟，或從磁碟載入腳本檔案。腳本編輯器提供語法檢查和命令執行功能，方便您管理儲存系統。如需更多資訊，請參閱 link:https://docs.netapp.com/us-en/e-series-santricity/sm-support/use-script-editor.html["使用腳本編輯器"^]。
...
此版本僅包含缺陷修復，不包含任何新功能。有關此版本變更的詳細資訊，請參閱 E-Series SANtricity OS 11.90R5 README。
...
`-identifyDevices` 參數
 a| 
SMcli 中現在提供了一個新的 `-identifyDevices`參數。此新參數可讓您尋找與儲存陣列相關聯的所有 SCSI 原生區塊裝置。如需詳細資訊，請參閱link:https://docs.netapp.com/us-en/e-series-cli/get-started/downloadable-smcli-parameters.html#identify-Devices["可下載的 SMcli 命令列參數"^]。
...
乙太網路核心統計資料
 a| 
System Manager 中的「檢視 iSCSI 統計資料套件」頁面現在新增了「乙太網路核心統計資料」選項。此新選項可讓您檢視 iSCSI 裝置平台核心驅動程式的統計資料。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/e-series-santricity/sm-support/view-iscsi-statistics-packages-support.html["檢視 iSCSI 統計資料套件"^]。
