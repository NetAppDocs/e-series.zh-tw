...
* 兩個獨立的網路以實現高可用性。請務必使用 VLAN 或兩個獨立的網路，將 iSCSI 流量與其他類型的流量隔離到不同的網段。
...
.關於這項工作
本任務介紹如何從 Hardware 頁面存取 iSCSI 連接埠組態。您也可以從功能表存取組態：Settings[System > Configure iSCSI ports]。
...
. 從左側面板中，選擇功能表：Hardware[Controllers and components]。
. 按一下要設定iSCSI連接埠的控制器。
+
此時會出現控制器的內容功能表。
...
image::../media/50001_01_conf-win.gif[iSCSI 連接埠識別碼]
