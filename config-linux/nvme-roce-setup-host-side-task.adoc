...
.開始之前
您必須執行最新的相容 RHEL 9、RHEL 10、SLES 15 或 SLES 16 Service Pack 作業系統。請參閱 https://mysupport.netapp.com/matrix["NetApp 互通性對照表工具"^]以取得完整的最新需求清單。
...
NOTE: 如果您正在使用 NVIDIA HCA 並且希望使用外部 DOCA 驅動程式，請參閱link:https://developer.nvidia.com/doca-downloads["NVIDIA DOCA 說明文件"]以取得有關如何在此環境中正確安裝它的資訊。
...
+
建立包含以下內容的範例檔案  `/etc/sysconfig/network/ifcfg-eth4`，其中 MTU 設定為在不發生封包分片的情況下可以達到的最大值（通常為 9000）。
...
. 重新啟動主機。
+
.. 或者，您也可以在不重新啟動的情況下執行以下命令載入 `nvme_rdma` kernel：
+
[listing]
----
...
+
.接下來呢？
在主機上透過 RoCE 設定 NVMe 啟動器後，link:nvme-roce-configure-storage-connections-task.html["設定 NVMe over RoCE 連接埠設定"]。
