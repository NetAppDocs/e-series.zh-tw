...
---
= 驗證 E 系列（ NVMe over roce ）的 Linux 組態支援與審查限制
:allow-uri-read: 
:experimental: 
:icons: font
:imagesdir: ../media/
...
CAUTION: *資料遺失風險*。您必須在無損環境中執行 NVMe over RoCE。請在您的網路上啟用端對端流量控制（使用全域暫停訊框或優先權流量控制 (PFC)），以消除資料遺失的風險。PFC 要求將 NVMe over RoCE 標記為優先權 3 流量,DSCP 值為 `106`。
...
.接下來呢？
在您驗證 Linux 組態支援並檢閱限制之後，link:nvme-roce-configure-ip-address-dhcp-task.html["使用 DHCP 設定 IP 位址"]。
